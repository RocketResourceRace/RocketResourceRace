...
public class ResourceManagementTable extends Element {
    ...
    public ResourceManagementTable(int x, int y, int w, int h) {
      ...
      rowGap = ceil(TEXTSIZE/4*JSONManager.loadFloatSetting("text scale"));
    ...
    public void draw(PGraphics canvas) {
      ...
      for (int i = 0; i < rows; i++) {
        ...
        if (page == 1) {
          ...
          canvas.text(
            JSONManager.getEquipmentClass(JSONManager.getEquipmentTypeClassFromID(names.get(page).get(i))[0]),
