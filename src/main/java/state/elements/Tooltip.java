...
public class Tooltip extends Element {
    ...
    public float maxWidthLine(ArrayList<String> lines) {
        ...
        for (int i=0; i<lines.size(); i++) {
            if (papplet.textWidth(lines.get(i)) > ml) {
                ml = papplet.textWidth(lines.get(i));
            }
        }
        return ml;
    }
    ...
    public void draw(PGraphics panelCanvas) {
        if (visible && text.length() > 0) {
            ...
            int tx = round(between(0, papplet.mouseX-xOffset-tw/2, papplet.width-tw));
            int ty = round(between(0, papplet.mouseY-yOffset+20, papplet.height-th-20));
