...
class Game extends State{
  ...
  void makeTaskAvailable(int task){
    ((DropDown)getElement("tasks", "party management")).makeAvailable(jsManager.gameData.getJSONArray("tasks").getJSONObject(task).getString("id"));
  ...
  void checkTasks(){
    ...
    for(int i=0; i<gameData.getJSONArray("tasks").size(); i++){
      js = jsManager.gameData.getJSONArray("tasks").getJSONObject(i);
    ...
    ((TaskManager)getElement("tasks", "party management")).select(gameData.getJSONArray("tasks").getJSONObject(parties[cellY][cellX].getTask()).getString("id"))
  }
      ...
      for (int x=0; x<mapWidth; x++){
        if (parties[y][x] != null){
          if (parties[y][x].player == turn){
            ...
            if (action != null){
              if (!(action.type==JSONIndex(jsManager.gameData.getJSONArray("tasks"), "Construction Mid")) && !(action.type==JSONIndex(gameData.getJSONArray("tasks"), "Construction End")))
              ...
              if (action.type==JSONIndex(jsManager.gameData.getJSONArray("tasks"), "Construction Mid")){
                  buildings[y][x].image_id = 1;
                  action = null;
              } else if(action.type==JSONIndex(jsManager.gameData.getJSONArray("tasks"), "Construction End")){
            ...
            if (action != null){
              ...
              parties[y][x].changeTask(JSONIndex(jsManager.gameData.getJSONArray("tasks"), "Rest"));
            }
            moveParty(x, y);
          }
          else {
  ...
  void elementEvent(ArrayList<Event> events){
    for (Event event : events){
      ...
      if (event.type == "valueChanged"){
        if (event.id == "tasks"){
          postEvent(new ChangeTask(cellX, cellY, JSONIndex(gameData.getJSONArray("tasks"), ((TaskManager)getElement("tasks", "party management")).getSelected())));
        }
      }
  ...
  boolean startInvalid(PVector p1, PVector p2){
    if(p1.dist(p2)<mapWidth/8||noise(p1.x*MAPNOISESCALE, p1.y*MAPNOISESCALE)<jsManager.loadFloatSetting("water level")||noise(p2.x*MAPNOISESCALE, p2.y*MAPNOISESCALE)<jsManager.loadFloatSetting("water level")){
      return true;
    }
    return false;
  }
