...
class BaseFileManager extends Element{
      ...
      if (saveNames[i].length() > 8) {// 'Untitled is 8 characters
        if (saveNames[i].substring(0, 8).equals("Untitled")){
          try{
            ...
            LOGGER.log(Level.SEVERE, "No autogen names available", e);
          }
        }
      }
    }
    String name = "Untitled"+(mx+1);
    LOGGER.info("Created autogenerated file name: " + name);
    return name;
  }
  ...
  void loadSaveNames(){
    try{
      ...
      if (!dir.exists()){
        LOGGER.info("Creating new 'saves' directory");
        dir.mkdir();
      }
      saveNames = dir.list();
    }
    catch (Exception e) {
      LOGGER.log(Level.SEVERE, "Files scanning failed", e);
    }
  }
  ...
  String selectedSaveName(){
    if (saveNames.length == 0){
      ...
      LOGGER.severe("Selected name is out of range " + selected);
    }
    LOGGER.info("Selected save name is : " + saveNames[selected]);
