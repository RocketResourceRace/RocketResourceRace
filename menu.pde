    ...
    for (Event event:events){
      if (event.type.equals("valueChanged")){
        ...
        if (event.id.equals("ground spawns")){
          ...
          changeSetting("ground spawns", ""+groundSpawns);
          writeSettings();
        }
      ...
      if (event.type.equals("toggled")){
        if (event.id.equals("map 3d")){
          mapIs3D = (boolean)((ToggleButton)getElement("map 3d", "new game")).getState();
          changeSetting("map3D", ""+mapIs3D);
          writeSettings();
        }
      }
